...
Program : ListStmt { $$ = make_Prog($1); result->program_ = $$; }
;
Stmt : _IDENT_ _EQ Exp { $$ = make_SAss($1, $3); result->stmt_ = $$; }
  | Exp { $$ = make_SExp($1); result->stmt_ = $$; }
;
...
;
Exp1 : Exp2 _PLUS Exp1 { $$ = make_ExpAdd($1, $3); result->exp_ = $$; }
  | Exp2 { $$ = $1; result->exp_ = $$; }
;
Exp2 : Exp2 _MINUS Exp3 { $$ = make_ExpSub($1, $3); result->exp_ = $$; }
  | Exp3 { $$ = $1; result->exp_ = $$; }
;
Exp3 : Exp3 _STAR Exp4 { $$ = make_ExpMul($1, $3); result->exp_ = $$; }
  | Exp3 _SLASH Exp4 { $$ = make_ExpDiv($1, $3); result->exp_ = $$; }
  | Exp4 { $$ = $1; result->exp_ = $$; }
;
Exp4 : _INTEGER_ { $$ = make_ExpLit($1); result->exp_ = $$; }
  | _IDENT_ { $$ = make_ExpVar($1); result->exp_ = $$; }
